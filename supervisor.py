...
class supervisor(object):
    ...
    def predict(self, input, events=None, model_restore_path=None):
        ...
        if str(self.device) == "cpu":
            ...
            TensorFloat = torch.FloatTensor
        X = TensorFloat(input).to(self.device)
        self.events = EventList(events=events)
        self.events.set_model(self)
...
class State(object):
    ...
    def set_data_train(self, X, y, batch_size, shuffle=True):
        if str(self.device) == "cpu":
            ...
            TensorFloat = torch.FloatTensor
        X, y = TensorFloat(X).to(self.device), TensorFloat(y).to(self.device)
    ...
    def set_data_val(self, X, y, num_val):
        ...
        if str(self.device) == "cpu":
            ...
            TensorFloat = torch.FloatTensor
        self.X_val, self.y_val = TensorFloat(X).to(self.device), TensorFloat(y).to(self.device)
    ...
    def set_data_test(self, X, y):
        ...
        if str(self.device) == "cpu":
            ...
            TensorFloat = torch.FloatTensor
        self.X_test, self.y_test = TensorFloat(X).to(self.device), TensorFloat(y).to(self.device)
